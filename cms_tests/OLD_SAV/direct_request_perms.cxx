...
int main ()
{
   ...
   const string   USR_LOGIN     ("q_request");
   const string   USR_PASSWD    ("q_request");

   const string   REQUEST_QUEUE ("queue.service");
   const string   MESSAGE       ("Ceci est un test de requête directe sur une queue de service");

   clog << "\nDIRECT REQUEST PERMS\n" ;
   bool           ok;
   ...
   vh_user_perms.vire_add_direct_request_perms ();
   ...
   chan.basic_publish ("", REQUEST_QUEUE, MESSAGE.c_str (), prop_out);
   while (1) {
